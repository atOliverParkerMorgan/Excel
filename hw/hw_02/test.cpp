...
CBigInt::CBigInt(const char * val) {
    ...
    std::string number = val + !is_positive;
...
CBigInt operator*(const CBigInt &val1, const CBigInt &val2) {
    ...
    while (index1 > 0) {
        int current = static_cast<int>(val1.value[--index1] - '0');
        int overflow = 0;
        index2 = length2;
        ...
        while (index2 > 0 || overflow) {
            int current2 = (index2 > 0) ? static_cast<int>(val2.value[--index2] - '0')
                                        : 0;
            int prod = current * current2 + overflow;
            overflow = prod / 10;
            prod %= 10;
            result_local.value.insert(0, 1, static_cast<char>(prod + '0'));
        }
...
bool operator<(const CBigInt &lhs, const CBigInt &rhs) {
    if (lhs.value != "0" && rhs.value != "0" &&
        ((lhs.is_positive && !rhs.is_positive) || (!lhs.is_positive && rhs.is_positive))) {
