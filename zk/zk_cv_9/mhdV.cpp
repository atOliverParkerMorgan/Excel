...
private:
    map<string, set<string>> allStops;
};

void CTransport::AddLine(istream &is) {
    string first, last, previous, currentName;
    while (getline(is, currentName)) {
        if (!previous.empty()) {
            allStops[previous].insert(currentName);
        }
        if (first.empty()) first = currentName;
        previous = currentName;
        last = currentName;
    }
    if (!first.empty()) allStops[last].insert(first);
}

map<string, int> CTransport::FindDest(const string &from, int costMax) const {
    queue<pair<int, string>> q;
    map<string, bool> visited;
    for (auto &it: allStops) {
    ...
    while (!q.empty()) {
        auto [currentCost, currentStopName] = q.front();
        q.pop();

        if (!result.contains(currentStopName) || result[currentStopName] > currentCost) {
            result[currentStopName] = currentCost;
        }

        if (!allStops.contains(currentStopName)) continue;

        for (const string &neighbour: allStops.at(currentStopName)) {
            if (!visited[neighbour] && currentCost + 1 <= costMax) {
                visited[neighbour] = true;
                q.emplace(currentCost + 1, neighbour);
            }
        }
    }
